## BFO upgrades
⬇️  Below are collapsible sections that you can click to open
<details>
<summary id="generalPMVupgrades">Upgrade all PMVs</summary>

### Applies to [PMV Scout](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMV_Scout) and [PMVA1 (T) Bloxian](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMVA1_(T)_Bloxian)
The PMV Scout is slower than the [WNAF](https://naramo-nuclear-plant-v2.fandom.com/wiki/West_Noobian_Assault_Force) [PMVA1 (T)](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMVA1_(T)) and weaker than an unarmred/unarmored [PMV](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMV).
The PMVA1 (T) Bloxian is slower than the same and just as capable WNAF [PMVA1 (T)](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMVA1_(T)).
- > Give the PMV Scout atleast 25 health ([Scout upgrades here](#scoutUpgrades)) and give both the PMV Scout and PMVA1 (T) Bloxian a top speed of 40.
![ ](/staticContent/scoutAndA1HP.webp)
</details>
<details>
<summary id="bfoM52">M52 APC Bloxian UPGRADES</summary>
 
### Applies to [M52 APC Bloxian](https://naramo-nuclear-plant-v2.fandom.com/wiki/M52_APC_Bloxian)
- > Update all BFO M52 variant chassis to the modern M52 chassis.
![ ](/staticContent/m52upgrades.gif)
- > Give all BFO M52 variants heavy-armor[^1] since the bloxian foriegn oversight is more technically advanced.
![ ](/staticContent/m52armor.png)
- > Give the M52 AMR APC 200 instead of 120 Health. (same chassis, different health doesn't make sense and also make people hate the vehicle)
![ ](/staticContent/m52amrhealthbuff.png)
- > Give the BFO M52s smoke grenades.
![ ](/staticContent/smokeGrenadesM52.png)
</details>
<details>
<summary id="scoutUpgrades">PMV Scout upgrades</summary>

### Applies to [PMV Scout](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMV_Scout)
- As powerful as the PMV Scout would be with these upgrades, It would still be as fragile as a [PMVA1 (T)](https://naramo-nuclear-plant-v2.fandom.com/wiki/PMVA1_(T)).
- Give the PMV Scout all these upgrades at the cost of it's extremely low XP requirement.
- Give it a top speed of 40.
- > Give it the PMVA1 armor package.
![ ](/staticContent/PMVA1.webp)
- > The M80 is too weak and inaccurate so make it's stats equal to the [M80 AMR](https://naramo-nuclear-plant-v2.fandom.com/wiki/M80_AMR)/[M52 AMR APC Bloxian](https://naramo-nuclear-plant-v2.fandom.com/wiki/M52_AMR_APC_Bloxian).
The [B63](https://naramo-nuclear-plant-v2.fandom.com/wiki/B63) doesn't perform great on vehicles so replace it with the well-known [M2HB](https://naramo-nuclear-plant-v2.fandom.com/wiki/M2HB).
![ ](/staticContent/pmvScout.webp)

</details>

[^1]: "Heavy-armor" is a term I made up for the stronger armor found on the [M52 IFV](https://naramo-nuclear-plant-v2.fandom.com/wiki/M52_IFV), [M52 APC "Garbage"](https://naramo-nuclear-plant-v2.fandom.com/wiki/M52_APC_%22Garbage%22), and the [M52A2 APC](https://naramo-nuclear-plant-v2.fandom.com/wiki/M52A2_APC) that weaken shots taken at it by 15% (50 DMG -> 42 DMG)
